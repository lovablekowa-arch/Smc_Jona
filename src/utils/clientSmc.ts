  ...
  return CLIENT_PAIRS_CATALOG.map((pair, idx) => {
    ...
    return {
      ...
      confluences: {
        ...
        condition2_FVG_OB: {
          satisfied: true,
          fvgSequenceM30M15Confirmed: true,
          fvgM30: {
            type: isBuy ? ('BULLISH' as const) : ('BEARISH' as const),
            timeframe: '30M' as const,
            high: isBuy ? fvgHigh * 1.002 : fvgHigh * 0.998,
            low: isBuy ? fvgLow * 0.998 : fvgLow * 1.002,
            sizePercent: 0.45,
            sizePoints: Number(((fvgHigh - fvgLow) * 1.2).toFixed(4)),
            mitigated: false,
            ageHours: 1.8,
            label: `FVG 30M Macro Intermédiaire 1.8h NON MITIGÉ (Taille: 0.45% | POC: ${pocPrice})`,
            isRecent: true,
            isAncient: false,
            isSignificant: true,
            stdevRatio: 1.45,
          ...
          fvgM15: {
            ...
            mitigated: false,
            ageHours: 1.4,
            label: `FVG 15M Précision 1.4h NON MITIGÉ (Zone d'Entrée & POC: 0.38% | POC: ${pocPrice})`,
            isRecent: true,
            isAncient: false,
            isSignificant: true,
            stdevRatio: 1.25,
          ...
          entryTapInStatus: isPriceInside ? ('TESTING_POC' as const) : ('CONFIRMED_INSIDE' as const),
          macroFvgH4: {
            type: isBuy ? ('BULLISH' as const) : ('BEARISH' as const),
            timeframe: '4H' as const,
            high: isBuy ? currentPrice * 0.994 : currentPrice * 1.014,
            low: isBuy ? currentPrice * 0.988 : currentPrice * 1.008,
            sizePercent: 0.65,
            sizePoints: Number((currentPrice * 0.006).toFixed(4)),
            mitigated: false,
            ageHours: 14.5,
            label: 'FVG H4 Macro non mitigé en soutien institutionnel',
            isRecent: false,
            isAncient: false,
            isSignificant: true,
          },
          macroFvgDaily: {
            ...
            high: isBuy ? currentPrice * 0.985 : currentPrice * 1.028,
            low: isBuy ? currentPrice * 0.975 : currentPrice * 1.018,
            sizePercent: 1.05,
            sizePoints: Number((currentPrice * 0.01).toFixed(4)),
            mitigated: false,
            ageHours: 48.0,
            label: 'FVG Daily (1D) Macro aligné',
            isRecent: false,
            isAncient: true,
            isSignificant: true,
          },
          macroFvgInformativeSummary: `💡 Confirmation Macro (Informatif) : FVG H4 (${(isBuy ? currentPrice * 0.988 : currentPrice * 1.008).toFixed(2)} - ${(isBuy ? currentPrice * 0.994 : currentPrice * 1.014).toFixed(2)}) + FVG Daily 1D alignés en renfort institutionnel HTF.`,
          ...
          summary: 'Suite FVG M30 & M15 VALIDÉE ✅ (Entrée M15/M30 avec POC fort) + FVG H4/Daily informatifs alignés',
        },
